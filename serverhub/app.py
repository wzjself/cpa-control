...
def cpa_summary(target: dict[str, Any], live: bool = False) -> dict[str, Any]:
    if not live:
        ...
        if snapshot:
            ...
            snapshot['invalid_401'] = int(snapshot.get('invalid_401') or sum(1 for r in accounts if r.get('invalid_401')))
            snapshot['quota_limited'] = int(snapshot.get('quota_limited') or sum(1 for r in accounts if r.get('quota_limited')))
            snapshot['abnormal'] = int(snapshot.get('abnormal') or sum(1 for r in accounts if (not r.get('invalid_401')) and (not r.get('quota_limited')) and str(r.get('status') or '').lower() in {'error', 'exception', 'abnormal', 'unknown'}))
            snapshot['healthy'] = int(snapshot.get('healthy') or sum(1 for r in accounts if not r.get('invalid_401') and not r.get('quota_limited') and not r.get('disabled') and str(r.get('status') or '').lower() not in {'error', 'exception', 'abnormal', 'unknown'}))
