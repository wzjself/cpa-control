...
function chartLabels(history) {
  return history.map(item => {
    ...
    if (currentRange === '7d' || currentRange === '30d' || currentRange === 'all') return `${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
...
function drawSimpleLineChart(canvas, datasets, opts = {}) {
  ...
  const pad = { top: 12, right: 10, bottom: 24, left: 34 }, w = cssWidth - pad.left - pad.right, h = cssHeight - pad.top - pad.bottom;
  if (w <= 10 || h <= 10) return;
  const values = datasets.flatMap(ds => ds.data || []).filter(v => Number.isFinite(v)); if (!values.length) return;
  let minY = Number.isFinite(opts.minY) ? opts.minY : Math.min(...values); let maxY = Number.isFinite(opts.maxY) ? opts.maxY : Math.max(...values);
  if (minY === maxY) { minY -= 1; maxY += 1; } if (opts.beginAtZero) minY = Math.min(0, minY);
  ctx.strokeStyle = 'rgba(158,177,206,0.18)'; ctx.lineWidth = 1;
  for (let i = 0; i <= 4; i++) { const y = pad.top + (h * i / 4); ctx.beginPath(); ctx.moveTo(pad.left, y); ctx.lineTo(pad.left + w, y); ctx.stroke(); }
  ctx.strokeStyle = 'rgba(158,177,206,0.35)'; ctx.beginPath(); ctx.moveTo(pad.left, pad.top); ctx.lineTo(pad.left, pad.top + h); ctx.lineTo(pad.left + w, pad.top + h); ctx.stroke();
  const toX = (idx, total) => pad.left + (total <= 1 ? 0 : (w * idx / (total - 1))); const toY = val => pad.top + h - ((val - minY) / (maxY - minY)) * h;
  datasets.forEach(ds => { const data = ds.data || []; ctx.strokeStyle = ds.color || '#60a5fa'; ctx.lineWidth = 2; ctx.beginPath(); data.forEach((v, i) => { const x = toX(i, data.length), y = toY(Number(v) || 0); if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y); }); ctx.stroke(); });
  ctx.fillStyle = '#9eb1ce'; ctx.font = '12px Inter, system-ui, sans-serif'; ctx.textAlign = 'right'; ctx.textBaseline = 'middle';
  for (let i = 0; i <= 4; i++) { const v = maxY - ((maxY - minY) * i / 4), y = pad.top + (h * i / 4); ctx.fillText(`${Math.round(v * 10) / 10}`, pad.left - 6, y); }
  const labels = opts.labels || [], tickCount = Math.min(6, labels.length);
  if (tickCount > 0) { ctx.textAlign = 'center'; ctx.textBaseline = 'top'; for (let i = 0; i < tickCount; i++) { const idx = Math.round((labels.length - 1) * (tickCount === 1 ? 0 : i / (tickCount - 1))); ctx.fillText(String(labels[idx] || ''), toX(idx, labels.length), pad.top + h + 6); } }
}
function renderChartFallback(canvas, message) {
  if (!canvas) return; const parent = canvas.parentElement; if (!parent) return;
  let note = parent.querySelector('.chart-fallback');
  if (!note) { note = document.createElement('div'); note.className = 'muted chart-fallback'; note.style.marginTop = '8px'; parent.appendChild(note); }
...
function renderUsageChart(historyRaw) {
  const history = downsample(historyRaw, currentRange === '3h' ? 90 : currentRange === '24h' ? 120 : 160); const canvas = document.getElementById('usageChart');
  if (!canvas || !history.length) { renderChartFallback(canvas, '暂无历史数据'); return; }
  clearChartFallback(canvas); destroyCanvasChart(canvas);
  drawSimpleLineChart(canvas, [{ label: 'CPU', data: history.map(x => Number(x.cpu_percent) || 0), color: '#60a5fa' }, { label: '内存', data: history.map(x => Number(x.mem_percent) || 0), color: '#34d399' }, { label: '磁盘', data: history.map(x => Number(x.disk_percent) || 0), color: '#f59e0b' }], { labels: chartLabels(history), minY: 0, maxY: 100, beginAtZero: true, height: 240 });
}
function seriesRatePerSecond(history, key) {
  ...
  for (let i = 1; i < history.length; i++) {
    ...
    const diffMb = Math.max(0, curVal - prevVal), seconds = (curTs - prevTs) / 1000; out.push(seconds > 0 ? (diffMb * 1024) / seconds : 0);
  }
  return out;
}
function renderTrafficChart(historyRaw) {
  const history = downsample(historyRaw, currentRange === '3h' ? 90 : currentRange === '24h' ? 120 : 160); const canvas = document.getElementById('trafficChart');
  if (!canvas || history.length < 2) { renderChartFallback(canvas, '暂无历史速率数据'); return; }
  clearChartFallback(canvas); destroyCanvasChart(canvas);
  const rx = seriesRatePerSecond(history, 'net_rx_mb'), tx = seriesRatePerSecond(history, 'net_tx_mb'), labels = chartLabels(history).slice(1), allVals = rx.concat(tx);
  drawSimpleLineChart(canvas, [{ label: '下载速率', data: rx, color: '#22c55e' }, { label: '上传速率', data: tx, color: '#f472b6' }], { labels, minY: 0, maxY: Math.max(1, ...allVals), height: 240 });
...
function renderRelayStats(data) {
  ...
  const items = [['今日请求', fmtNum(data.requests_today)], ['今日 Token', fmtNum(data.tokens_today)], ['RPM 峰值', fmtNum(data.rpm_peak)], ['TPM 峰值', fmtNum(data.tpm_peak)], ['近 24h Key 数', fmtNum(data.keys_24h)], ['近 24h 模型数', fmtNum(data.models_24h)]];
...
function renderCpaCard(cpa) {
  ...
  return `
    <div class="cpa-card" data-cpa-id="${cpa.id}">
      <div class="cpa-head"><div><div><strong>${cpa.name}</strong> <span class="badge">${cpa.provider}</span></div><div class="muted">${cpa.base_url}</div></div><div class="cpa-actions"><button class="ghost small-btn" onclick="refreshCpa('${cpa.id}')">刷新当前 CPA</button><button class="ghost small-btn" onclick="delete401('${cpa.id}')">一键删除401</button><button class="danger small-btn" onclick="deleteCpa('${cpa.id}')">删除 CPA</button></div></div>
      <div class="mini-grid"><div class="mini-stat"><div class="muted">总凭证</div><div class="n">${fmtNum(cpa.total)}</div></div><div class="mini-stat"><div class="muted">401</div><div class="n err">${fmtNum(cpa.invalid_401)}</div></div><div class="mini-stat"><div class="muted">limit</div><div class="n warn">${fmtNum(cpa.quota_limited)}</div></div><div class="mini-stat"><div class="muted">健康</div><div class="n ok">${fmtNum(cpa.healthy)}</div></div></div>
      <div class="quota-row"><div class="quota-label"><span>总凭证额度使用情况</span><span>${cpa.remaining_ratio === null || cpa.remaining_ratio === undefined ? `已知额度 ${knownQuotaAccounts} 个 · 未知 ${unknownQuotaAccounts} 个` : `剩余 ${fmtMaybePct(cpa.remaining_ratio)} / 已用 ${fmtMaybePct(cpa.used_ratio)} · 未知 ${unknownQuotaAccounts} 个`}</span></div><div class="progress remain"><span style="width:${cpa.remaining_ratio ?? 0}%"></span></div></div>
      <div class="muted" style="margin:10px 0">当前凭证状态（实时读取 CPA 后台）</div>
      <div class="account-grid compact-account-grid">${(cpa.accounts || []).map(acc => `<div class="account-chip"><div class="account-chip-top"><strong title="${esc(acc.email || acc.name)}">${esc(acc.email || acc.name)}</strong><span class="${accountStatusClass(acc)}">${accountStatusText(acc)}</span></div><div class="progress mini-remain"><span style="width:${acc.remaining_ratio ?? 0}%"></span></div><div class="muted">剩余 ${fmtMaybePct(acc.remaining_ratio)}</div><div class="muted">${acc.quota_checked_at ? `更新 ${fmtTime(acc.quota_checked_at)}` : '未取到时间'}</div><div class="account-actions"><button class="danger small-btn" onclick="deleteAuthFile('${cpa.id}', '${encodeURIComponent(acc.name || acc.email || '')}')">删除</button></div></div>`).join('') || '<div class="muted">暂无状态数据，点“刷新并扫描全部 CPA”重试。</div>'}</div>
    </div>`;
}
...
function renderServer(server) {
  els.healthScore.textContent = server.health; els.cpuNow.textContent = fmtPct(server.latest.cpu_percent); els.memNow.textContent = fmtPct(server.latest.mem_percent); els.diskNow.textContent = fmtPct(server.latest.disk_percent); els.trafficNow.textContent = `${fmtMb(server.traffic_24h.rx_mb)} / ${fmtMb(server.traffic_24h.tx_mb)}`;
  els.usageChartTitle.textContent = `CPU / 内存 / 磁盘（${rangeLabel(currentRange)}）`; els.trafficChartTitle.textContent = `网络上行 / 下载历史（${rangeLabel(currentRange)}）`; setActiveRangeBtn(); renderProcesses(server.top_processes || []);
...
async function loadServerStatus(forceBust = false) {
  const url = `/api/server-status?range=${encodeURIComponent(currentRange)}${forceBust ? `&_t=${Date.now()}` : ''}`; const res = await fetch(url, { cache: 'no-store' }); const data = await res.json(); renderServer(data.server);
}
...
async function loadAll(forceBust = false) {
  const url = `/api/overview?range=${encodeURIComponent(currentRange)}${forceBust ? `&_t=${Date.now()}` : ''}`; const res = await fetch(url, { cache: 'no-store' }); const data = await res.json(); renderServer(data.server); renderRelayStats(data.clirelay || {}); renderCpas(data.cpas || []); await loadCredentials();
}
...
function parseCredentialBlocks(text, prefix = '') {
  ...
  return parts.map((part, idx) => ({ name: `${prefix || '凭证'}-${idx + 1}`, filename: `${prefix || 'credential'}-${Date.now()}-${idx + 1}.json`, content: part }));
}
async function importCredentials() {
  const items = parseCredentialBlocks(els.credentialBulkInput.value, els.credentialNamePrefix.value.trim()); if (!items.length) return alert('先粘贴凭证内容');
  const res = await fetch('/api/credentials/import', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ items }) }); const data = await res.json(); els.credentialBulkInput.value = ''; renderCredentialStore(data.credentials || [], latestCpas); toast(`已导入 ${data.saved?.length || 0} 个凭证`);
}
async function deploySelectedCredentials() {
  const targetId = els.credentialTargetSelect.value, ids = Array.from(selectedCredentialIds); if (!targetId) return alert('先选择目标 CPA'); if (!ids.length) return alert('先勾选仓库里的凭证');
  ...
  const data = await res.json(); if (data.cpa) replaceCpaCard(data.cpa); renderCredentialStore(data.credentials || [], latestCpas);
  const okCount = (data.results || []).filter(x => x.ok).length, failItems = (data.results || []).filter(x => !x.ok);
  failItems.forEach(x => selectedCredentialIds.add(x.id));
  (data.results || []).filter(x => x.ok).forEach(x => selectedCredentialIds.delete(x.id));
  toast(failItems.length ? `成功 ${okCount} 个，失败 ${failItems.length} 个：${failItems.slice(0,3).map(x => x.name).join('、')}` : `已成功投放 ${okCount} 个凭证`);
}
