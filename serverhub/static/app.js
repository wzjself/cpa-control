...
async function toggleKeepalive(id, enabled, button) { const run = async () => { const res = await fetch(`/api/cpas/${id}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ keepalive_enabled: enabled }) }); const data = await res.json(); if (!res.ok) return alert(data.error || '设置失败'); if (data.cpa) replaceCpaCard(data.cpa); }; return withButtonLoading(button, enabled ? '开启中...' : '关闭中...', run)(); }
async function runKeepaliveProbe(id, button) { const run = async () => { const res = await fetch(`/api/cpas/${id}/keepalive-probe`, { method:'POST' }); const data = await res.json(); if (data.cpa) replaceCpaCard(data.cpa); if (!res.ok) return alert(data.error || '保活探测失败'); const ok = (data.results || []).filter(x => x.ok).length; alert(`保活探测完成：成功 ${ok} / ${(data.results || []).length}`); }; return withButtonLoading(button, '探测中...', run)(); }
...
window.deleteCpa = deleteCpa; window.refreshCpa = refreshCpa; window.toggleKeepalive = toggleKeepalive; window.runKeepaliveProbe = runKeepaliveProbe; window.delete401 = delete401; window.batchDeleteByKind = batchDeleteByKind; window.exportByKind = exportByKind; window.deleteAuthFile = deleteAuthFile; window.toggleCredentialSelect = toggleCredentialSelect; window.deleteCredential = deleteCredential; window.toggleCpaExpand = toggleCpaExpand; window.renameCpa = renameCpa; window.moveCpa = moveCpa;
