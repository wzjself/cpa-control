...
function drawSimpleLineChart(canvas, datasets, opts = {}) {
  if (!canvas) return;
  const dpr = Math.max(1, window.devicePixelRatio || 1), cssWidth = Math.max(320, canvas.clientWidth || canvas.parentElement?.clientWidth || 600), cssHeight = Math.max(220, opts.height || 240);
  canvas.width = Math.floor(cssWidth * dpr); canvas.height = Math.floor(cssHeight * dpr); canvas.style.width = `${cssWidth}px`; canvas.style.height = `${cssHeight}px`;
  ...
  let minY = Number.isFinite(opts.minY) ? opts.minY : Math.min(...values), maxY = Number.isFinite(opts.maxY) ? opts.maxY : Math.max(...values);
  if (minY === maxY) { minY -= 1; maxY += 1; } if (opts.beginAtZero) minY = Math.min(0, minY);
  ctx.strokeStyle = 'rgba(158,177,206,0.18)'; ctx.lineWidth = 1; for (let i = 0; i <= 4; i++) { const y = pad.top + (h * i / 4); ctx.beginPath(); ctx.moveTo(pad.left, y); ctx.lineTo(pad.left + w, y); ctx.stroke(); }
  ...
  const toX = (idx, total) => pad.left + (total <= 1 ? 0 : (w * idx / (total - 1))), toY = val => pad.top + h - ((val - minY) / (maxY - minY)) * h;
  ...
  ctx.fillStyle = '#9eb1ce'; ctx.font = '12px Inter, system-ui, sans-serif'; ctx.textAlign = 'right'; ctx.textBaseline = 'middle'; for (let i = 0; i <= 4; i++) { const v = maxY - ((maxY - minY) * i / 4), y = pad.top + (h * i / 4); ctx.fillText(`${Math.round(v * 10) / 10}`, pad.left - 6, y); }
  const labels = opts.labels || [], tickCount = Math.min(6, labels.length); if (tickCount > 0) { ctx.textAlign = 'center'; ctx.textBaseline = 'top'; for (let i = 0; i < tickCount; i++) { const idx = Math.round((labels.length - 1) * (tickCount === 1 ? 0 : i / (tickCount - 1))); ctx.fillText(String(labels[idx] || ''), toX(idx, labels.length), pad.top + h + 6); } }
}
...
function mergeImportItems() {
  const textItems = parseCredentialBlocks(els.credentialBulkInput.value, els.credentialNamePrefix.value.trim());
  return [...pendingFileItems, ...textItems];
}
async function importCredentials() {
  const items = mergeImportItems();
  if (!items.length) return alert('先粘贴凭证内容或选择凭证文件');
...
async function readSelectedFiles(files) {
  const prefix = els.credentialNamePrefix?.value.trim();
  pendingFileItems = await Promise.all(Array.from(files || []).map(async file => {
  ...
  toast(`已选择 ${pendingFileItems.length} 个凭证文件，点“导入到仓库”即可入库`);
}
