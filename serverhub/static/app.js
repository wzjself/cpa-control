...
async function saveCpaOrder() { const ids = getCpaOrderIds(); if (!ids.length) return; await fetch('/api/cpas/reorder', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ ids }) }); }
function bindCpaDragEvents() { Array.from(els.cpaList.querySelectorAll('.cpa-card')).forEach(card => { card.addEventListener('dragstart', () => { draggingCpaId = card.dataset.cpaId; card.classList.add('dragging'); }); card.addEventListener('dragend', async () => { card.classList.remove('dragging'); draggingCpaId = null; await saveCpaOrder(); }); card.addEventListener('dragover', e => { e.preventDefault(); const dragging = els.cpaList.querySelector('.cpa-card.dragging'); if (!dragging || dragging === card) return; const rect = card.getBoundingClientRect(); const before = (e.clientY - rect.top) < rect.height / 2; els.cpaList.insertBefore(dragging, before ? card : card.nextSibling); }); }); }
...
window.deleteCpa = deleteCpa; window.refreshCpa = refreshCpa; window.delete401 = delete401; window.deleteAuthFile = deleteAuthFile; window.toggleCredentialSelect = toggleCredentialSelect; window.deleteCredential = deleteCredential; window.toggleCpaExpand = toggleCpaExpand; window.renameCpa = renameCpa;
