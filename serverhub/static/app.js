...
async function saveCpaAuthToStore(cpaId, encodedName, button) { const run = async () => { const res = await fetch(`/api/cpas/${cpaId}/auth-files/${encodedName}/save-to-store`, { method:'POST' }); const data = await res.json(); if (!res.ok) return showUiNotice('上传仓库失败', [data.error || '入仓失败']); await loadAll(true); const added = Number(data.saved?.length || 0); const skipped = Number(data.skipped?.length || 0); const deduped = Number(data.dedupe_removed || 0); showUiNotice('上传仓库完成', ['成功 ' + added + ' 个', '重复 ' + skipped + ' 个', '失败 0 个', deduped > 0 ? ('清理历史重复 ' + deduped + ' 个') : '']); }; return withButtonLoading(button, '入仓中...', run)(); }

function exportSingleCpaAuth(cpaId, encodedName) { window.open(`/api/cpas/${cpaId}/auth-files/${encodedName}/export`, '_blank'); }
async function saveSelectedCpaAuths(cpaId, button) { const items = getSelectedCpaAuthItems(cpaId); if (!items.length) return showUiNotice('提示', ['先勾选这个 CPA 的凭证']); const run = async () => { let added = 0, skipped = 0, deduped = 0, failed = 0; const failedNames = []; for (const item of items) { try { const res = await fetch(`/api/cpas/${item.cpa_id}/auth-files/${encodeURIComponent(item.file_name)}/save-to-store`, { method:'POST' }); const data = await res.json(); if (!res.ok) { failed += 1; failedNames.push(item.file_name); continue; } added += Number(data.saved?.length || 0); skipped += Number(data.skipped?.length || 0); deduped += Number(data.dedupe_removed || 0); } catch (e) { failed += 1; failedNames.push(item.file_name); } } Array.from(selectedCpaAuthIds).filter(key => key.startsWith(`${cpaId}::`)).forEach(key => selectedCpaAuthIds.delete(key)); await loadAll(true); showUiNotice('上传仓库完成', ['成功 ' + added + ' 个', '重复 ' + skipped + ' 个', '失败 ' + failed + ' 个', deduped > 0 ? ('清理历史重复 ' + deduped + ' 个') : '', failedNames.length ? ('失败项：' + failedNames.slice(0, 3).join('、') + (failedNames.length > 3 ? ' ...' : '')) : '']); }; return withButtonLoading(button, '上传中...', run)(); }
