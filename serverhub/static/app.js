...
async function loadCredentials() { const res = await fetch('/api/credentials', { cache: 'no-store' }); const data = await res.json(); renderCredentialStore(data.credentials || [], data.cpas || []); if (els.credentialStoreHint && Number(data.dedupe_removed || 0) > 0) els.credentialStoreHint.textContent = `已清理 ${data.dedupe_removed} 个重复凭证`; }
...
async function syncCredentialStatus(options = {}) { const { silent = false, button = els.syncCredentialStatusBtn } = options; const run = async () => { const targetId = els.credentialTargetSelect?.value || ''; if (!targetId) { if (!silent) alert('当前没有可用的目标 CPA'); return; } const targetName = els.credentialTargetSelect?.selectedOptions?.[0]?.text || '当前目标'; if (els.credentialStoreHint) els.credentialStoreHint.textContent = `正在用缓存名单匹配 ${targetName} ...`; const res = await fetch('/api/credentials/sync-upload-status', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ target_id: targetId }) }); const data = await res.json(); if (!res.ok) { if (els.credentialStoreHint) els.credentialStoreHint.textContent = data.error || '同步失败'; if (!silent) alert(data.error || '同步失败'); return; } renderCredentialStore(data.credentials || [], data.cpas || latestCpas); if (els.credentialStoreHint) els.credentialStoreHint.textContent = `当前目标：${targetName} · 缓存匹配到 ${data.matched || 0} 个`; if (!silent) alert(`状态已更新：当前 CPA 匹配到 ${data.matched || 0} 个凭证`); }; if (button) return withButtonLoading(button, '更新中...', run)(); return run(); }
