...
async function saveSelectedCpaAuths(cpaId, button) { const items = getSelectedCpaAuthItems(cpaId); if (!items.length) return showUiNotice('提示', ['先勾选这个 CPA 的凭证']); const run = async () => { let added = 0, skipped = 0, deduped = 0, failed = 0; const failedNames = []; appendProgressLog('上传仓库', `开始处理 0/${items.length} 个凭证`, 'info'); for (let index = 0; index < items.length; index += 1) { const item = items[index]; appendProgressLog('上传仓库', `进度 ${index + 1}/${items.length} · ${item.file_name}`, 'info'); try { const res = await fetch(`/api/cpas/${item.cpa_id}/auth-files/${encodeURIComponent(item.file_name)}/save-to-store`, { method:'POST' }); const data = await res.json(); if (!res.ok) { failed += 1; failedNames.push(item.file_name); appendProgressLog('上传仓库', `${item.file_name} 失败`, 'error'); continue; } added += Number(data.saved?.length || 0); skipped += Number(data.skipped?.length || 0); deduped += Number(data.dedupe_removed || 0); appendProgressLog('上传仓库', `${item.file_name} 完成：成功 ${Number(data.saved?.length || 0)}，重复 ${Number(data.skipped?.length || 0)}`, 'success'); } catch (e) { failed += 1; failedNames.push(item.file_name); appendProgressLog('上传仓库', `${item.file_name} 异常：${e}`, 'error'); } } Array.from(selectedCpaAuthIds).filter(key => key.startsWith(`${cpaId}::`)).forEach(key => selectedCpaAuthIds.delete(key)); appendProgressLog('上传仓库', `全部完成：成功 ${added}，重复 ${skipped}，失败 ${failed}`, failed ? 'warn' : 'success'); await loadAll(true); showUiNotice('上传仓库完成', ['成功 ' + added + ' 个', '重复 ' + skipped + ' 个', '失败 ' + failed + ' 个', deduped > 0 ? ('清理历史重复 ' + deduped + ' 个') : '', failedNames.length ? ('失败项：' + failedNames.slice(0, 3).join('、') + (failedNames.length > 3 ? ' ...' : '')) : '']); }; return withButtonLoading(button, '上传中...', run)(); }
...
async function deleteSelectedCpaAuths(cpaId, button) { const items = getSelectedCpaAuthItems(cpaId); if (!items.length) return showUiNotice('提示', ['先勾选这个 CPA 的凭证']); if (!confirm(`确认删除这 ${items.length} 个凭证吗？`)) return; const run = async () => { let failed = 0; appendProgressLog('删除 CPA 凭证', `开始删除 0/${items.length} 个凭证`, 'warn'); for (let index = 0; index < items.length; index += 1) { const item = items[index]; appendProgressLog('删除 CPA 凭证', `进度 ${index + 1}/${items.length} · ${item.file_name}`, 'warn'); const res = await fetch(`/api/cpas/${item.cpa_id}/auth-files/${encodeURIComponent(item.file_name)}`, { method:'DELETE' }); const data = await res.json(); if (!res.ok) { failed += 1; continue; } } Array.from(selectedCpaAuthIds).filter(key => key.startsWith(`${cpaId}::`)).forEach(key => selectedCpaAuthIds.delete(key)); appendProgressLog('删除 CPA 凭证', `完成：成功 ${items.length - failed}，失败 ${failed}`, failed ? 'warn' : 'success'); await loadAll(true); showUiNotice('删除完成', ['成功 ' + (items.length - failed) + ' 个', '失败 ' + failed + ' 个']); }; return withButtonLoading(button, '删除中...', run)(); }
...
async function importCredentialFiles(files, button) { const fileList = Array.from(files || []).filter(Boolean); if (!fileList.length) return; const run = async () => { appendProgressLog('上传文件', `开始导入 ${fileList.length} 个文件`, 'info'); const items = await Promise.all(fileList.map(async file => ({ name: file.name, filename: file.name, content: await file.text() }))); const importRes = await fetch('/api/credentials/import', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ items }) }); const importData = await importRes.json(); if (els.credentialFilesInput) els.credentialFilesInput.value = ''; if (!importRes.ok) { appendProgressLog('上传文件', importData.error || '入仓失败', 'error'); return showUiNotice('上传文件失败', [importData.error || '入仓失败']); } renderCredentialStore(importData.credentials || [], latestCpas); const added = Number(importData.saved?.length || 0); const skipped = Number(importData.skipped?.length || 0); const deduped = Number(importData.dedupe_removed || 0); appendProgressLog('上传文件', `完成：新增 ${added}，重复 ${skipped}${deduped > 0 ? '，清理历史重复 ' + deduped : ''}`, 'success'); showUiNotice('上传文件完成', ['新增 ' + added + ' 个', '重复 ' + skipped + ' 个', deduped > 0 ? ('清理历史重复 ' + deduped + ' 个') : '']); }; return withButtonLoading(button, '上传中...', run)(); }

async function deploySelectedCredentials() { const run = async () => { const targetId = els.credentialTargetSelect?.value || '', ids = Array.from(selectedCredentialIds); if (!targetId) return showUiNotice('提示', ['当前没有可用的目标 CPA']); if (!ids.length) return showUiNotice('提示', ['先勾选仓库里的凭证']); appendProgressLog('仓库→CPA', `开始投放 0/${ids.length} 个仓库凭证到目标 CPA`, 'info'); const res = await fetch('/api/credentials/deploy', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ target_id: targetId, credential_ids: ids }) }); const data = await res.json(); if (data.cpa) replaceCpaCard(data.cpa); renderCredentialStore(data.credentials || [], latestCpas); const summary = summarizeDeployResults(data.results || []); summary.failItems.forEach(x => selectedCredentialIds.add(x.id)); summary.okItems.forEach(x => selectedCredentialIds.delete(x.id)); markRecentlyUploaded(summary.okItems.map(x => x.id)); appendProgressLog('仓库→CPA', summary.message, summary.failItems.length ? 'warn' : 'success'); showUiNotice('批量上传到目标 CPA', [summary.message]); }; return withButtonLoading(els.deploySelectedBtn, '上传中...', run)(); }
