...
const els = {
  ...
  diskUsedFreeNow: document.getElementById('diskUsedFreeNow'),
...
function renderServer(server, relay = {}) { els.healthScore.textContent = server.health; els.cpuNow.textContent = fmtPct(server.latest.cpu_percent); els.memNow.textContent = fmtPct(server.latest.mem_percent); els.diskNow.textContent = fmtPct(server.latest.disk_percent); els.diskUsedFreeNow.textContent = `${fmtGb(server.disk_usage?.used_gb)} / ${fmtGb(server.disk_usage?.free_gb)}`; els.netRateNow.textContent = `${fmtKbps(server.current_net_kbps?.rx)} / ${fmtKbps(server.current_net_kbps?.tx)}`; els.trafficNow.textContent = `${fmtMb(server.traffic_24h.rx_mb)} / ${fmtMb(server.traffic_24h.tx_mb)}`; els.relayRequestNow.textContent = fmtNum(relay.request_count || 0); els.relayTokenNow.textContent = fmtNum(relay.total_tokens || 0); els.relayRateNow.textContent = `${fmtNum(relay.rpm || 0)} / ${fmtNum(relay.tpm || 0)}`; els.usageChartTitle.textContent = `CPU / 内存 / 磁盘（${rangeLabel(currentRange)}）`; els.trafficChartTitle.textContent = `网络上行 / 下载历史（${rangeLabel(currentRange)}）`; setActiveRangeBtn(); try { renderUsageChart(server.history || []); } catch (e) { console.error('usage chart render failed', e); } try { renderTrafficChart(server.history || []); } catch (e) { console.error('traffic chart render failed', e); } }
async function loadServerStatus(forceBust = false) { const serverUrl = `/api/server-status?range=${encodeURIComponent(currentRange)}${forceBust ? `&_t=${Date.now()}` : ''}`; const [serverRes, overviewRes] = await Promise.all([fetch(serverUrl, { cache: 'no-store' }), fetch(`/api/overview?range=${encodeURIComponent(currentRange)}${forceBust ? `&_t=${Date.now()}` : ''}`, { cache: 'no-store' })]); const serverData = await serverRes.json(); const overviewData = await overviewRes.json(); renderServer(serverData.server, overviewData.clirelay || {}); }
