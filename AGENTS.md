...
修改任何正在运行的项目代码、配置、服务脚本或部署文件后：

1. 必须重启对应进程 / 服务 / 容器，使修改生效
2. 必须做一次最小验证（如端口监听、HTTP 200、systemctl active、docker ps）
3. 回复用户时要明确说明：改了什么、是否已重启、验证结果如何

除非用户明确要求不要重启，或重启会带来明显风险且尚未获得确认，否则不要只改文件不重启。
